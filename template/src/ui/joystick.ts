 ...
 * common failure of virtual sticks on phones. A faint hint ring sits at the home
 * position while idle so the control is discoverable (including for a viewer
 * watching a recording).
 *
 * Drawn with primitives (`Graphics`), so it scales with `TUNING.joystick` and
 * re-skins with `PALETTE` — no art asset, no nine-slice.
 *
...
export class Joystick {
  ...
  private setIdleAlpha(): void {
    this.base.setAlpha(TUNING.joystick.idleAlpha);
    this.knob.setAlpha(TUNING.joystick.idleAlpha);
  }

  private paintBase(): void {
    this.base.clear();
    this.base.fillStyle(PALETTE.bgDeep, 0.45);
    this.base.fillCircle(0, 0, this.radius);
    this.base.lineStyle(4, PALETTE.primary, 0.8);
    this.base.strokeCircle(0, 0, this.radius);
    this.base.lineStyle(2, PALETTE.primary, 0.35);
  ...
  private paintKnob(): void {
    this.knob.clear();
    this.knob.fillStyle(PALETTE.primary, 0.9);
    this.knob.fillCircle(0, 0, this.knobRadius);
    this.knob.lineStyle(3, PALETTE.ink, 0.9);
