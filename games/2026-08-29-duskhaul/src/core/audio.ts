...
const VOICES = {
  ...
  /**
   * PRD §12's two authored voices — the only two the template does not ship,
   * and the two moments the whole extraction axis turns on. Parameters are
   * verbatim from the §12 table, not retuned by ear: a rising square fifth for
   * a door opening, and a 1.6s noise-led sub falling 60→30Hz for the Collapse
   * igniting. Both are one-per-event, so the long decays cost nothing.
   */
  gate: { wave: 'square', freq: 110, freqEnd: 220, attack: 0.02, decay: 0.9, gain: 0.5 },
  collapse: {
    wave: 'sine',
    freq: 60,
    freqEnd: 30,
