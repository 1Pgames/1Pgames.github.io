...
echo "== content contract check =="
# The failure class `tsc` structurally cannot see: a TUNING key the code reads
# but config.ts does not define, a hazard/behaviour param `systems/zone.ts` or
# `objects/enemy.ts` requires but `src/data/**` does not author, a wave that
# spawns an id the roster dropped, or a Collapse tuned so its ring closes over
# Gate C. Every one of those is a hard runtime failure at RUN START with a
# green typecheck, which is exactly how this build lost a day.
