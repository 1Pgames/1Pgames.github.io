...
// --- with every v1 field preserved and every later field defaulted.
// ---
// --- The version is read off DEFAULT_META, never written as a literal: the
// --- invariant is "a v1 save lands on the CURRENT version", and a hardcoded
// --- number turns every legitimate schema bump into a false failure here
// --- instead of catching a broken migration chain. (It already did: v2 -> v3
// --- added `stash`, `gear` and `stats.wardenKills`.)
{
  store.clear();
  const v1 = {
    version: 1,
  ...
  // Every v1 stat survives verbatim; counters added later default rather than
  // overwrite, which is the whole point of an additive migration.
  assert.equal(migrated.stats.runs, v1.stats.runs, 'lifetime runs survive migration');
  assert.equal(migrated.stats.wins, v1.stats.wins, 'lifetime wins survive migration');
  assert.equal(migrated.stats.bestScore, v1.stats.bestScore, 'best haul survives migration');
  assert.equal(migrated.stats.bestTimeMs, v1.stats.bestTimeMs, 'best time survives migration');
  assert.equal(migrated.stats.wardenKills, 0, 'wardenKills defaults to zero');
  ...
  assert.deepEqual(migrated.stash, [], 'the relic stash defaults to empty');
  assert.deepEqual(
    migrated.gear,
    { blade: null, shroud: null, trinket: null },
    'every gear slot defaults to nothing equipped',
  );
