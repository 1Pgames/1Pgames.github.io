 ...
 * reads the attacker's `damageMul` / `critChance` / `critMul` stat keys so a
 ...
 */
const WALL_CLOCK: () => number =
  typeof performance !== 'undefined' ? () => performance.now() : () => Date.now();

let damageClock: () => number = WALL_CLOCK;
...
/**
 * Hands the clock back to wall time. A scene that installed a closure over its
 * own sim time MUST call this on SHUTDOWN: the module-level clock outlives the
 * scene, so a stale closure keeps a dead scene's field alive and — worse —
 * reports a FROZEN time to the next scene's `Health`, which makes every i-frame
 * window permanent (see `slices/arena/game.ts` `teardown`).
 */
export function resetDamageClock(): void {
  damageClock = WALL_CLOCK;
}
...
/**
 * Rolls one hit: the attacker's authored `baseDamage` scaled by the frozen
 * §16.1 `damageMul` stat, then `critChance` (0..1) to multiply by `critMul`
 * (default 2x if the stat is absent).
 *
 * `baseDamage` is passed rather than read off the block because a weapon's base
 * damage is authored PER WEAPON (`data/weapons.ts` / `TUNING.weapons.*`) — the
 * stat surface carries the multiplier, never the base (see `config.ts`
 * `PLAYER_BASE_STATS`).
 */
export function rollDamage(
  ...
  const base = baseDamage * stats.get('damageMul');
